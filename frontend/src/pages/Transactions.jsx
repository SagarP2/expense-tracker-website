    ...
    <Card className="p-4 bg-surface border-border shadow-sm">
      <div className="flex flex-col gap-4">
        <div className="flex justify-between items-start">
          <div>
            <p className="font-bold text-text text-base truncate max-w-[200px]">{t.description || t.category}</p>
          ...
          <span
            className={clsx(
              'font-bold text-base flex items-center gap-1 px-2.5 py-1 rounded-lg shrink-0',
        ...
        <div className="flex items-center justify-between pt-3 border-t border-border/50">
          <Badge variant="outline" className="bg-neutral-50 border-neutral-200 px-2.5 py-1 text-xs">
            {t.category}
          </Badge>
          <div className="flex gap-1">
            <button onClick={(e) => { e.stopPropagation(); handleEdit(t); }} className="p-2 text-text-muted hover:text-primary hover:bg-primary/10 rounded-lg transition-colors">
              <Edit2 size={16} />
            </button>
            <button onClick={(e) => { e.stopPropagation(); setDeleteDialog({ isOpen: true,id: t._id }); }} className="p-2 text-text-muted hover:text-danger hover:bg-danger/10 rounded-lg transition-colors">
              <Trash2 size={16} />
            </button>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-surface/40 backdrop-blur-md p-4 rounded-3xl border border-white/20 shadow-sm">
        <div>
          <h2 className="text-2xl sm:text-3xl font-bold text-text tracking-tight">Transactions</h2>
          <p className="text-text-muted mt-1 font-medium text-sm sm:text-base">Manage your financial records</p>
        </div>
        <div className="flex gap-3 w-full sm:w-auto">
          <Button
            ...
            className="flex-1 sm:flex-none items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 bg-gradient-to-r from-emerald-500 to-teal-600 dark:bg-none dark:bg-emerald-600 hover:from-emerald-600 hover:to-teal-700 text-white border-none transition-all hover:-translate-y-0.5"
          >
            <Plus size={18} />
            Add Income
          </Button>
          <Button
            ...
            className="flex-1 sm:flex-none items-center justify-center gap-2 shadow-lg shadow-rose-500/20 bg-gradient-to-r from-rose-500 to-pink-600 dark:bg-none dark:bg-rose-600 hover:from-rose-600 hover:to-pink-700 text-white border-none transition-all hover:-translate-y-0.5"
          >
            <Plus size={18} />
            Add Expense
          </Button>
      ...
      {/* Filters Section */}
      <div className="flex flex-col sm:flex-row gap-4 bg-surface/30 p-2 rounded-2xl border border-white/40 shadow-sm">
        <div className="relative flex-1 group">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted group-focus-within:text-primary transition-colors" size={20} />
          <input
            type="text"
            placeholder="Search transactions..."
            className="w-full pl-11 pr-4 py-3 rounded-xl border-none bg-surface/50 focus:bg-surface focus:outline-none focus:ring-2 focus:ring-primary/10 shadow-sm transition-all placeholder:text-text-muted/70 text-text"
            value={filter.search}
        ...
        <div className="relative">
          ...
          <input
            type="month"
            className="pl-11 pr-4 py-3 rounded-xl border-none bg-surface/50 
               focus:bg-surface focus:outline-none focus:ring-2 focus:ring-primary/10 
               shadow-sm transition-all cursor-pointer text-sm font-medium text-text min-w-[160px] w-full sm:w-auto"
            value={filter.month}
        ...
        <div className="relative">
          <Filter className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted pointer-events-none" size={18} />
          <select
            className="pl-11 pr-10 py-3 rounded-xl border-none bg-surface/50 focus:bg-surface focus:outline-none focus:ring-2 focus:ring-primary/10 shadow-sm transition-all cursor-pointer text-sm font-medium text-text appearance-none min-w-[140px] w-full sm:w-auto"
            value={filter.type}
            ...
            {!lockedType && (
              <div className="grid grid-cols-2 gap-4 p-1.5 bg-neutral-50 rounded-2xl border border-neutral-100">
                <label className="cursor-pointer">
                  ...
                  />
                  <div className="text-center py-3 rounded-xl text-sm font-bold text-text-muted transition-all peer-checked:bg-white peer-checked:text-rose-600 peer-checked:shadow-sm">
                    Expense
                  </div>
                </label>
                ...
                <label className="cursor-pointer">
                  <input
                    ...
                    className="hidden peer"
                  />
                  <div className="text-center py-3 rounded-xl text-sm font-bold text-text-muted transition-all peer-checked:bg-white peer-checked:text-emerald-600 peer-checked:shadow-sm">
                    Income
                  </div>
                </label>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                {formData.category === '__other__' ? (
                  <input
                    ...
                    required
                    className="w-full px-4 py-3 rounded-xl border border-border bg-surface focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-medium"
                  />
                ) : (
                  <div className="relative">
                    <select
